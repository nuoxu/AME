...
class EpochBasedRunner(BaseRunner):
    ...
    def train(self, data_loader, **kwargs):
        ...
        if self.epoch % 5 == 4:
            with tune.checkpoint_dir(step=self.epoch + 1) as checkpoint_dir:
                self.save_checkpoint(os.path.join(checkpoint_dir, "checkpoint"))
    ...
    def save_checkpoint(self,
        ...
        save_checkpoint(self.model, out_dir, optimizer=optimizer, meta=meta)
...
def start_trial(config, checkpoint_dir=None, args=None):

    cfg = Config.fromfile(os.path.join(base_dir, "configs/cifar10/resnet34_b16x8.py"))
    ...
    if checkpoint_dir:
        cfg.resume_from = os.path.join(checkpoint_dir, "checkpoint")
...
def main():
    ...
    scheduler = PopulationBasedTraining(
        ...
        hyperparam_mutations = {
            ...
            "weight_decay": lambda: random.uniform(0.0, 0.001),
        },
    )
    ...
    np.save(osp.join(work_dir, 'pbt_results.npy'), result_dict)
