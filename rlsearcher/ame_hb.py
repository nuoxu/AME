...
class TuneAME_HB(Searcher):
    ...
    Tune automatically converts search spaces to TuneAME_HB's format:
    ...
    .. code-block:: python
        ...
        algo = TuneAME_HB(max_concurrent=4, metric="mean_loss", mode="min")
        ame = HyperBandForAME(
    ...
    def __init__(self,
        ...
        super(TuneAME_HB, self).__init__(metric=self._metric, mode=mode)
    ...
    def on_trial_result(self, trial_id: str, result: Dict):
        ...
        if "hyperband_info" not in result:
            logger.warning("AME Info not detected in result. Are you using "
                           "HyperBandForAME as a scheduler?")
        elif "budget" in result.get("hyperband_info", {}):
            hbs_wrapper = self.to_wrapper(trial_id, result)
            update_model = result["hyperband_info"]["budget"] == 1
            self.amer.new_result(hbs_wrapper, update_model)
    ...
    def convert_search_space(spec: Dict) -> ConfigSpace.ConfigurationSpace:
        ...
        def resolve_value(par: str, domain: Domain
            ...
            raise ValueError("TuneAME_HB does not support parameters of type "
